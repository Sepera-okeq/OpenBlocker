...
public class ConfigLimitedUtils {
	...
	public static void readFromFile(String par1FileName) {
		...
		if ((new File(filePath)).exists()) {
			try {
				...
				Main.getInstance().getListLimited().clear();
				String[] text = (str).split("@");

				for (int i = 0; i < text.length; ++i) {
					Main.getInstance().getListLimited().add(text[i]);
				}
				...
				Logger.getLogger(ConfigLimitedUtils.class.getName()).log(Level.SEVERE, null, var8);
			} catch (IOException var9) {
				Logger.getLogger(ConfigLimitedUtils.class.getName()).log(Level.SEVERE, null, var9);
			}
		} else {
			try {
				(new File(filePath)).createNewFile();
	...
	public static void deleteFromFile(String par1FileName, String strDelete) {
		...
		if ((new File(filePath)).exists()) {
			try {
				...
				Logger.getLogger(ConfigLimitedUtils.class.getName()).log(Level.SEVERE, null, var10);
			}
		} else {
			try {
				(new File(filePath)).createNewFile();
